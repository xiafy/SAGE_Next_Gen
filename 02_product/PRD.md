...
**Icebreaker 机制**：
- 图片确认发送后立即进入 AgentChat，不等识别完成
- AI 立即发出 Icebreaker（如"菜单正在识别中～先告诉我你们几位用餐？有什么忌口吗？"）
- 利用识别等待时间收集用户上下文，减少焦虑感
- 识别完成后 AI 自动衔接，给出开场介绍 + 推荐
...
**验收标准**：
- AC1: AI Icebreaker 在进入 AgentChat 后 < 1 秒出现
- AC2: AI 正式推荐响应 < 5 秒（P90，菜单识别完成后）
- AC3: 快捷按钮由 AI 根据上下文动态生成
- AC4: 切换到探索视图再返回，对话历史完整保留
- AC5: 网络中断时，已有对话不丢失
...
| # | 问题 | 影响 | 优先级 |
|---|------|------|--------|
| OQ1 | 免费试用次数 X = 多少？ | 定价策略 | Sprint 2 前 |
| OQ3 | Vision 模型选型 | 识别质量/成本 | Sprint 1 技术选型 |
| OQ4 | 5 张图同时提交还是逐张？ | API 成本/体验 | Sprint 1 技术选型 |
