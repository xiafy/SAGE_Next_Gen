...
## DEC-079: 任务模板治理能力恢复（2026-03-06）

- **日期**: 2026-03-06
- **决策人**: 夏总
- **背景**: DEC-076 将 TASK_TEMPLATE.md 拆分为 IMPL + TEST 两份模板，解决了认知独立性问题，但拆分时把"职责分离"和"内容精简"混为一谈，导致原模板 60% 的治理能力丢失（前置检查、AC 清单、DEC 冲突检查、契约断言、反模式等）。
- **决策**:
  1. 两份模板各自承载完整治理框架，不抽象 BASE 层
  2. 重复不是成本（DEC-078），治理缺失才是
  3. 职责边界处分化：IMPL 不写测试 + Spec 偏离清单；TEST 禁读源码 + 质量红线
  4. 流程变为：SAGE 填 IMPL → 实现 Agent 交付 → SAGE 审查 → SAGE 填 TEST → 测试 Agent 交付 → SAGE 审查 → 门控
...
## DEC-081: AC 测试分类与有效性度量（2026-03-09）

**背景**: Batch 1-3 写了 74 个 AC 测试，脚本报告 72% 覆盖率，但夏总质疑"100% 通过是否有水分"。逐条审查发现大量测试标了 AC 但实际验证层级不匹配。

**决策**:

1. **AC 按 ABCDE 五类分类**：
   - A 逻辑型（20个）：单元测试能证明 → 有标签即有效
   - B 组件行为型（22个）：需渲染组件 → 测试必须含 render/screen 等
   - C 集成型（5个）：需跨模块 mock → 测试必须含 mock/spy/fetch 等
   - D AI 质量型（3个）：需 prompt 断言 → 测试必须含 prompt/instruction 等
   - E 性能/硬件（10个）：排除自动化分母

2. **度量脚本输出双指标**：tagged（标签存在）+ valid（类型匹配）
3. **分类注册表**: `scripts/ac-registry.json`，每条 AC 一个分类

**修正结果**:
- 脚本报告值: 43/60 = 72%（虚高）
- 修正后: tagged 43/53, valid 32/53 = 60%（诚实）
- 诚实-严格（人工评估）: 24/50 = 48%

**核心教训**: "测试 100% 通过"不等于"AC 被验证"。测试在验证"代码做了什么"而非"代码应该做什么"时，通过率越高越是假象。
