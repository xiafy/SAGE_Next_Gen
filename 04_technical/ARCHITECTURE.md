...
> 版本: v1.0
> 日期: 2026-02-25
> 状态: 🟡 骨架版本，OQ3/OQ4 待明日决策后补全
> 上游文档: `02_product/PRD.md v1.2`、`DECISIONS.md`
...
│     AI 模型 API         │  │    外部数据 API                 │
│                        │  │                                │
│ Vision: [TBD - OQ3]    │  │  GPS: 浏览器 Geolocation API   │
│ Chat:   claude-sonnet  │  │  天气: Open-Meteo（免费）        │
│ Fallback: glm-5/kimi   │  │        Sprint 2 实现            │
└────────────────────────┘  └────────────────────────────────┘

部署：
  前端 → Cloudflare Pages（CDN 全球加速）
  Worker → Cloudflare Workers（固定东京节点，规避 Gemini 中国区限制）
```
...
| 场景 | 模型 | 理由 |
|------|------|------|
| 菜单识别（Vision）| **[TBD - OQ3]** | 需明日决策 |
| 对话推荐（Chat）| `claude-sonnet-4-6` | 质量/成本平衡 |
| 降级模式 | `glm-5` 或 `kimi` | API 异常时的备用 |
| ⚠️ 禁止 | Gemini 全系列 | 严重限速，已废弃 |
...
| 编号 | 问题 | 影响 | 解决时间 |
|------|------|------|---------|
| OQ3 | Vision 模型选型（Claude Vision vs 其他）| `/api/analyze` 实现 | 明日对齐 |
| OQ4 | 5 张图片是同时提交还是逐张？影响 Prompt 设计和 API 成本 | `/api/analyze` 接口 | 明日对齐 |
...
代码仓库（GitHub）
    ...
    │   └─ pnpm build → 静态文件部署到 CDN
    │   └─ 域名：sage-next-gen.pages.dev（正式上线后自定义域名）
    │
    └─► Cloudflare Workers CI/CD（wrangler deploy）
        └─ worker/ 目录构建 → 部署到 gcp:asia-northeast1（东京）
        └─ API Keys 通过 wrangler secret 管理
```
