...
## L6 — Prompt 行为测试（手动 / E2E 验证）

> 目标: 验证 System Prompt 规则在实际 AI 输出中的表现

---

### TC-L601: AI 不说"已为您下单"等越界用语（角色边界）

```
前置条件: 菜单已识别，进入对话
输入: 用户说"我要这道菜"
期望结果:
  - AI 回复不包含"已为您下单""订单已确认""开始准备""请稍等准备"
  - AI 回复类似"已加入点餐单，可以展示给服务员～"
  - recommendations 包含对应 itemId
优先级: P0
Sprint: Sprint 2
```

### TC-L602: 深夜时段 AI 不说"下午茶时光"等矛盾描述（时间事实一致性）

```
前置条件: timestamp 设置为 23:30（深夜）
输入: 用户说"推荐点什么"
期望结果:
  - AI 回复不包含"下午茶""适合午餐"等与深夜矛盾的时间描述
  - AI 可以提及"宵夜"或不提及时段
优先级: P1
Sprint: Sprint 2
```

### TC-L603: 深夜用户想点正餐，AI 正常推荐不拒绝（时间弱绑定）

```
前置条件: timestamp 设置为 23:00（深夜），菜单包含正餐菜品
输入: 用户说"我想吃个正餐，推荐一下"
期望结果:
  - AI 不拒绝推荐（不说"现在不是正餐时段"）
  - AI 正常推荐正餐菜品
优先级: P1
Sprint: Sprint 2
```

### TC-L604: quickReplies 不包含"开始准备""帮我下单"等越界选项

```
前置条件: 菜单已识别，进入对话
输入: 多轮对话后观察 quickReplies
期望结果:
  - quickReplies 不包含"开始准备""帮我下单""确认订单"等越界用语
  - quickReplies 均为用户视角（如"还有什么推荐""看看点餐单"）
优先级: P0
Sprint: Sprint 2
```

### TC-L605: 用户选 ≥3 道菜时，quickReplies 包含"看看点餐单"

```
前置条件: 用户已通过对话加入 3 道菜到点餐单
输入: 继续对话
期望结果:
  - quickReplies 中包含"看看点餐单"或"View my order"类似选项
优先级: P1
Sprint: Sprint 2
```

### TC-L606: 用户问"我点了什么"，AI 引导看📋而非文字复述
...
期望结果:
  - AI 不逐条列出已点菜品
  - AI 引导"点右上角📋查看点餐单"或类似引导语
优先级: P1
Sprint: Sprint 2
```

### TC-L607: AI 回复不超过 2 句话

```
前置条件: 菜单已识别，进入对话
输入: 多轮不同类型的对话消息
期望结果:
  - AI message 字段中句子数 ≤ 2（以句号/感叹号/问号计）
优先级: P1
Sprint: Sprint 2
```
...
## L7 — UI 行为测试（E2E / 组件测试补充）

> 目标: 验证 Part 2 UI 修正的行为正确性

---

### TC-L701: AgentChat Header 右上角无📷图标，只有📋和🛒

```
前置条件: 菜单已识别，进入 AgentChatView，chatPhase = chatting
输入: 观察 Header 右上角
期望结果:
  - 无📷相机图标按钮
  - 有📋浏览菜单按钮（当 menuData 存在时）
  - 有🛒点单按钮（当 orderItems > 0 时）
优先级: P0
Sprint: Sprint 2
```

### TC-L702: 左下角输入框旁📷可用，可进入 Scanner（Path C）

```
前置条件: chatPhase = chatting
输入: 点击输入框左侧📷按钮
期望结果:
  - 设置 isSupplementing = true
  - 导航至 scanner 视图
  - 可以正常拍照补充菜单
优先级: P0
Sprint: Sprint 2
...
### TC-L703: AI 推荐菜品显示「+ 加入」按钮

```
前置条件: AI 返回 recommendations 包含 2 道菜
输入: 观察推荐卡片
期望结果:
  - 每张推荐卡片右侧显示「+ 加入」按钮（中文）或「+ Add」按钮（英文）
  - 按钮可点击
优先级: P0
Sprint: Sprint 2
```

### TC-L704: 点击「+ 加入」后按钮变为 ✓已加入（灰色）
...
### TC-L705: Order Card 每道菜显示数量和 +/- 按钮

```
前置条件: orderItems 包含 2 道菜
输入: 打开 OrderCardView
期望结果:
  - 每道菜显示数量（默认 1）
  - 每道菜有 − 和 + 按钮
...
### TC-L706: Order Card 点 + 数量增加，点 − 数量减少

```
前置条件: 某道菜 quantity = 2
输入: 点击 + 按钮，再点击 − 按钮
期望结果:
  - 点 +：quantity 变为 3，小计价格更新
  - 点 −：quantity 变回 2，小计价格更新
  - 底部总价同步更新
优先级: P0
Sprint: Sprint 2
```

### TC-L707: Order Card 数量减至 0 或点 ✕ 时菜品被删除
...
### TC-L708: Order Card 底部显示总数量和预估总价

```
前置条件: orderItems 包含 3 道菜，各 quantity 不同
输入: 观察 OrderCardView 底部
期望结果:
  - 左侧显示总数量（如"共 5 道菜"）
  - 右侧显示预估总价（正确的货币格式）
  - 有"展示给服务员"按钮
...
### TC-L709: Order Card 空状态显示引导文案

```
前置条件: orderItems 为空
输入: 打开 OrderCardView
期望结果:
  - 中文显示"还没有加入菜品，去和 AI 聊聊吧～"
  - 英文显示"No dishes yet — chat with AI to get started!"
  - 有按钮可导航回聊天页面
优先级: P1
Sprint: Sprint 2
```

### TC-L710: 重复加入同一道菜，quantity +1 而非新增行

```
前置条件: orderItems 已包含菜品 A（quantity = 1）
输入: 再次通过推荐卡片加入菜品 A
期望结果:
  - orderItems 中菜品 A 的 quantity 变为 2（不是新增一行）
  - Order Card 中该菜品只显示一行，数量为 2
优先级: P0
Sprint: Sprint 2
```
